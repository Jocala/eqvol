...
class EQVStream: EQVObject {
  ...
  static func getPropertyDataSize (objectID: AudioObjectID? = nil, address: AudioObjectPropertyAddress) -> UInt32? {
    ...
    case kAudioStreamPropertyAvailableVirtualFormats,
         kAudioStreamPropertyAvailablePhysicalFormats: return UInt32(kEQVDeviceSupportedSampleRates.count) * Memory.sizeof(AudioStreamRangedDescription.self)
    default:
      return nil
    }
  }
  
  static func getPropertyData (objectID: AudioObjectID?, address: AudioObjectPropertyAddress, inData: UnsafeRawPointer?) -> EQVObjectProperty? {
    ...
    case kAudioStreamPropertyVirtualFormat,
      ...
      return .streamDescription(
        EQVDevice.getDescription(for: EQVDevice.sampleRate)
      )
    case kAudioStreamPropertyAvailableVirtualFormats,
         kAudioStreamPropertyAvailablePhysicalFormats:
      ...
      let formats = ContiguousArray(kEQVDeviceSupportedSampleRates.map { sampleRate in
        return AudioStreamRangedDescription(
          mFormat: EQVDevice.getDescription(for: sampleRate),
  ...
  static func setPropertyData(client: EQVClient?, objectID: AudioObjectID?, address: AudioObjectPropertyAddress, data: UnsafeRawPointer, changedProperties: inout [AudioObjectPropertyAddress]) -> OSStatus {
    switch address.mSelector {
    case kAudioStreamPropertyIsActive:
    ...
    case kAudioStreamPropertyVirtualFormat,
      ...
      if kEQVDeviceSupportedSampleRates.contains(newDescription.mSampleRate) {
      ...
      if EQVDevice.sampleRate != newDescription.mSampleRate {
