...
class EQVDevice: EQVObject {
  static var name = kEQVDeviceDefaultName
  static var sampleRate = kDefaultSampleRate
  static var running = false
  static var shown = true {
    didSet {
      NSLog("EQVDIAG: shown changed \(oldValue) -> \(shown)")
      if (oldValue != shown) {
        EQVDriver.propertiesUpdated(
  ...
  static func hasProperty (objectID: AudioObjectID? = nil, address: AudioObjectPropertyAddress) -> Bool {
    ...
    case kAudioObjectPropertyBaseClass,
         ...
         EQVDeviceCustom.properties.shown,
         EQVDeviceCustom.properties.version,
         EQVDeviceCustom.properties.latency,
         EQVDeviceCustom.properties.name:
      return true
  ...
  static func getPropertyDataSize (objectID: AudioObjectID? = nil, address: AudioObjectPropertyAddress) -> UInt32? {
    ...
    case kAudioDevicePropertyAvailableNominalSampleRates: return UInt32(kEQVDeviceSupportedSampleRates.count) * Memory.sizeof(AudioValueRange.self)
    ...
    case kAudioObjectPropertyCustomPropertyInfoList:
      return Memory.sizeof(AudioServerPlugInCustomPropertyInfo.self) * EQVDeviceCustom.properties.count
    case EQVDeviceCustom.properties.latency: return Memory.sizeof(CFNumber.self)
    case EQVDeviceCustom.properties.shown: return Memory.sizeof(CFBoolean.self)
    case EQVDeviceCustom.properties.version: return Memory.sizeof(CFString.self)
    case EQVDeviceCustom.properties.name: return Memory.sizeof(CFString.self)
  ...
  static func getPropertyData (objectID: AudioObjectID? = nil, address: AudioObjectPropertyAddress, inData: UnsafeRawPointer?) -> EQVObjectProperty? {
    switch address.mSelector {
    case kAudioObjectPropertyBaseClass:
    ...
    case kAudioDevicePropertyAvailableNominalSampleRates:
      ...
      return .valueRangeList(
        ContiguousArray(kEQVDeviceSupportedSampleRates.map { freq -> AudioValueRange in
    ...
    case kAudioDevicePropertyIsHidden:
      // WORKAROUND: the server-side custom-property marshaling for 'shwn'
      // (CFPropertyList-typed) mangles client writes, which can flip the
      // stored flag and hide the device. Always report visible instead.
      return .integer(0)
    ...
    case EQVDeviceCustom.properties.version:
    ...
    case EQVDeviceCustom.properties.shown:
      return .bool(shown ? kCFBooleanTrue : kCFBooleanFalse)
    default: return nil
    }
  }

  static func setPropertyData(client: EQVClient?, objectID: AudioObjectID? = nil, address: AudioObjectPropertyAddress, data: UnsafeRawPointer, changedProperties: inout [AudioObjectPropertyAddress]) -> OSStatus {
    switch address.mSelector {
    ...
    case kAudioDevicePropertyNominalSampleRate:
      ...
      if !kEQVDeviceSupportedSampleRates.contains(newSampleRate) {
      ...
      if (sampleRate != newSampleRate) {
        ...
        DispatchQueue.global(qos: .default).async {
          _ = EQVDriver.host?.pointee.RequestDeviceConfigurationChange(
            EQVDriver.host!,
            kObjectID_Device,
    ...
    // Custom Properties
    case EQVDeviceCustom.properties.shown:
      // Only allow eqVol app to set this property
    ...
    case EQVDeviceCustom.properties.latency:
      // Custom property: only EqVol drives this
      ...
      return noErr
    case EQVDeviceCustom.properties.name:
      var newName = data.load(as: CFString.self) as String
      NSLog("EQVDIAG: name write received, current=\(name) new=\(newName)")

      if name != newName {
        if newName == "" {
          newName = kEQVDeviceDefaultName
        }
        name = newName
  ...
  static func getZeroTimeStamp (outSampleTime: UnsafeMutablePointer<Float64>, outHostTime: UnsafeMutablePointer<UInt64>, outSeed: UnsafeMutablePointer<UInt64>) -> OSStatus {
    ...
    let hostTicksPerRingBuffer = EQVDriver.hostTicksPerFrame! * Float64(ringBufferSize)
  ...
  static func doIO (client: EQVClient?, operationID: UInt32, sample: UnsafeMutablePointer<Float32>, cycleInfo: AudioServerPlugInIOCycleInfo, frameSize: UInt32) -> OSStatus {
